...
function OrderSignPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      // 1) captura da imagem da assinatura
      const dataURL = sigRef.current.getTrimmedCanvas().toDataURL("image/png");
      const blob = dataURLToBlob(dataURL);
      const filePath = `${signerType}-${id}-${Date.now()}.png`;

      // 2) upload para Storage (bucket "signatures")
      ...
      if (uploadError) {
        ...
        setError(
          `Erro ao enviar assinatura. Detalhes: ${
            uploadError.message || "tente novamente."
          }`
        );
        return;
      }
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("signatures").getPublicUrl(uploadData.path);
      ...
      if (signerType === "client") {
        ...
        updatePayload.client_accept_text = `${TERMS_TEXT}\n\nAssinante: ${name} - Documento: ${document}`;
      } else {
      ...
      if (updateError) {
        ...
        setError(
          `Erro ao salvar assinatura. Detalhes: ${
            updateError.message || "tente novamente."
          }`
        );
        return;
      }
      ...
      // Redireciona para a página de impressão (rota SPA)
  ...
  return (
    <div className="form-card" style={{ maxWidth: 900, margin: "1rem auto" }}>
      ...
      <p>
        <strong>Nº OS:</strong> {order.order_number}
      </p>
      <p>
